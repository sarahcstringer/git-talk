...
Commands, configs, and bonus picks from "The Git Commands I Avoided for 9 Years" — Write the Docs Portland 2026.

If something here makes you go *"wait, that exists?"* — that's the talk working. Looking is safe.

## The three from the talk

### `git worktree` — a second checkout, cheap

Two folders, two branches, one repo. The `.git` directory is shared, so a worktree costs roughly nothing on disk — no duplicate commits, just a fresh checkout of working files.
...
Reach for it any time you'd otherwise stash, switch branches, come back, and pop. Especially good for review-while-you-code.

> A given branch can only be checked out in one worktree at a time. Git refuses double-checkout — that's a feature, not a bug.
...
> **Tip:** Commit often. Reflog only saves committed work — uncommitted edits aren't in the object store yet. Ugly WIPs are the price of admission to the safety net.
...
When did this sentence get added? Removed? The pickaxe finds commits that change the count of a string.
...
Use `-G "regex"` for regex matching instead of literal strings. The archaeology tool for docs people. Shipped 2006.

### `rerere` — reuse recorded resolution

Git remembers how you resolved a conflict; next time the same conflict shows up, it auto-applies the fix.

```bash
git config --global rerere.enabled true
```

Free to turn on. Pairs well with `--update-refs` — some conflicts still resurface across stack rebases. Shipped 2006.
...
`checkout` still works. The new ones are easier to teach.

### `git blame -L` — line-range blame

Blame a specific line range. Add `-w` to ignore whitespace-only changes — huge for prose with reformatting.
...
A different VCS that works against git repos. Rethinks the model: no staging area, every change is a snapshot, conflicts become first-class objects you can carry around and resolve later.

Not "switch to jj." Just: git isn't settled law.
...
The commands were never the problem. The fear was.
